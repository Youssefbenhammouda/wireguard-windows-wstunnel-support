...
func runAboutDialog(owner walk.Form) error {
	...
	detailsLbl.SetText(l18n.Sprintf("App version: %s (%s)\nDriver version: %s\nGo version: %s\nOperating system: %s\nArchitecture: %s", version.Number, version.BuildTag, driver.Version(), strings.TrimPrefix(runtime.Version(), "go"), version.OsName(), version.Arch()))
